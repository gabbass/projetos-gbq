...
import { publicSagazErrorMessage, SagazApiError } from "@/lib/sagaz/errors"
import { sendWhatsappText } from "@/lib/sagaz/whatsapp"
...
export async function POST(request: Request) {
  ...
  try { return Response.json(await sendWhatsappText({ to, text }), { status: 201 }) }
    ...
    console.error("[sagaz] Falha ao enviar texto", { code: error instanceof SagazApiError ? error.code : "UNKNOWN" })
    return Response.json({ error: publicSagazErrorMessage(error) }, { status: error instanceof SagazApiError && error.status === 429 ? 429 : 503 })
  }
}
