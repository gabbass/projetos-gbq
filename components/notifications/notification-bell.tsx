...
export function NotificationBell({ enabled }: { enabled: boolean }) {
  ...
  async function markRead(notification: AppNotification) {
    if (!notification.readAt) {
      ...
      await fetch("/api/notifications/read", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: notification.id }) })
    }
...
function NotificationSkeleton() {
  return <div className="space-y-1 p-4">{Array.from({ length: 4 }, (_, index) => <div key={index} className="flex gap-3 py-3"><Skeleton className="size-9 rounded-full" /><div className="flex-1 space-y-2"><Skeleton className="h-4 w-2/3" /><Skeleton className="h-3 w-full" /><Skeleton className="h-3 w-20" /></div></div>)}</div>
}
