    ...
    <CardContent className="space-y-3">
      ...
      {subtasks.length > 0 ? <div className="border-y py-1">
        <button type="button" aria-expanded={showSubtasks} aria-controls={subtasksId} onClick={() => setShowSubtasks((current) => !current)} className="flex w-full items-center gap-1.5 rounded-lg px-1 py-2 text-left text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
          ...
          <span className="flex-1">{subtasks.filter((item) => item.status === "done").length}/{subtasks.length} subtarefas concluídas</span>
        </button>
