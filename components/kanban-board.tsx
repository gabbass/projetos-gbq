...
function TaskCard({ task, dragging, onDragStart, onDragEnd }: { task: ProjectTask; dragging: boolean; onDragStart: (task: ProjectTask, event: React.DragEvent) => void; onDragEnd: () => void }) {
  return <Card size="sm" draggable onDragStart={(event) => onDragStart(task, event)} onDragEnd={onDragEnd} className={`cursor-grab bg-background shadow-sm transition-[opacity,transform,box-shadow] active:cursor-grabbing ${dragging ? "scale-[0.98] opacity-40 shadow-none" : "hover:-translate-y-0.5 hover:shadow-md"}`}><CardHeader><CardTitle className="pr-7 text-sm leading-snug">{task.title}</CardTitle><CardAction><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon-sm" aria-label={`Ações de ${task.title}`}><MoreHorizontal /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48"><DropdownMenuLabel>Ações</DropdownMenuLabel><DropdownMenuSeparator /><EditTaskSheet task={task} /><DropdownMenuSeparator /><DropdownMenuLabel>Mover para</DropdownMenuLabel>{columns.filter((column) => column.status !== task.status).map((column) => <form action={async () => { await moveTaskAction(task.id, column.status) }} key={column.status}><DropdownMenuItem asChild><button type="submit" className="w-full"><span className={`size-2 rounded-full ${column.dot}`} />{column.title}</button></DropdownMenuItem></form>)}</DropdownMenuContent></DropdownMenu></CardAction></CardHeader><CardContent className="space-y-3">{task.description ? <p className="line-clamp-2 text-xs leading-relaxed text-muted-foreground">{task.description}</p> : null}<div className="flex flex-wrap items-center justify-between gap-2"><span className="text-xs font-medium">{task.owner}</span><Badge variant={task.priority === "high" ? "destructive" : "outline"}>{priorityLabel[task.priority]}</Badge></div>{task.due_date ? <p className="flex items-center gap-1.5 border-t pt-3 text-xs text-muted-foreground"><Clock3 className="size-3.5" />{formatDate(task.due_date)}</p> : null}</CardContent></Card>
}

export function KanbanBoard({ projects, tasks }: { projects: Project[]; tasks: ProjectTask[] }) {
  ...
  return <div className="flex flex-col gap-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><div className="mb-1 flex items-center gap-2 text-sm text-muted-foreground"><CalendarDays className="size-4" />Tarefas organizadas por etapa</div><h1 className="font-heading text-2xl font-semibold tracking-tight md:text-3xl">Visão geral</h1><p className="mt-1 text-muted-foreground">Mova as tarefas entre as colunas para atualizar o avanço do projeto.</p></div>{project ? <div className="flex flex-col gap-2 sm:flex-row sm:items-center"><Select value={project.id} onValueChange={setSelectedId}><SelectTrigger className="w-full sm:w-64"><SelectValue /></SelectTrigger><SelectContent>{projects.map((item) => <SelectItem key={item.id} value={item.id}><FolderKanban />{item.name}</SelectItem>)}</SelectContent></Select><NewTaskSheet projectId={project.id} projectName={project.name} /></div> : null}</div>
    {!project ? <Card><CardContent className="flex min-h-80 flex-col items-center justify-center gap-3 text-center"><span className="flex size-12 items-center justify-center rounded-2xl bg-muted"><CircleDashed className="size-5 text-muted-foreground" /></span><div><p className="font-medium">Crie um projeto para começar</p><p className="text-sm text-muted-foreground">O Kanban ficará disponível assim que houver um projeto cadastrado.</p></div><Button asChild variant="outline"><a href="/projetos">Ir para Projetos<ChevronRight /></a></Button></CardContent></Card> : <><Card size="sm"><CardHeader><CardDescription>{project.area} · Responsável: {project.owner}</CardDescription><CardTitle>{project.name}</CardTitle><CardAction><Badge variant="outline">{completedTasks}/{projectTasks.length} concluídas · {projectProgress}%</Badge></CardAction></CardHeader></Card>{moveError ? <p role="alert" className="text-sm text-destructive">{moveError}</p> : null}<div className="grid items-start gap-4 md:grid-cols-2 2xl:grid-cols-4">{columns.map((column) => { const items = projectTasks.filter((task) => task.status === column.status); return <Card key={column.status} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; setDropTarget(column.status) }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDropTarget(null) }} onDrop={(event) => dropTask(column.status, event)} className={`min-h-72 transition-[box-shadow,transform] ${column.surface} ${dropTarget === column.status && draggingId ? "scale-[1.01] ring-2 ring-primary/50 ring-offset-2" : ""}`}><CardHeader><CardTitle className="flex items-center gap-2"><span className={`size-2.5 rounded-full ${column.dot}`} />{column.title}</CardTitle><CardAction><Badge variant="outline" className="bg-background/70">{items.length}</Badge></CardAction></CardHeader><CardContent className="space-y-3">{items.length ? items.map((task) => <TaskCard key={task.id} task={task} dragging={draggingId === task.id} onDragStart={beginDrag} onDragEnd={() => { setDraggingId(null); setDropTarget(null) }} />) : <div className={`flex min-h-32 items-center justify-center rounded-xl border border-dashed bg-background/30 px-4 text-center text-xs text-muted-foreground transition-colors ${dropTarget === column.status && draggingId ? "border-primary bg-primary/5 text-primary" : ""}`}>{draggingId ? "Solte a tarefa aqui" : "Nenhuma tarefa nesta etapa"}</div>}</CardContent></Card> })}</div></>}
  </div>
}
