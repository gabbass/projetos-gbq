...
function EditProjectSheet({ project, clients, responsibles }: { project: Project; clients: UserOption[]; responsibles: UserOption[] }) {
  ...
  return <Sheet>
    ...
    <SheetContent className="overflow-y-auto sm:max-w-lg"><SheetHeader><SheetTitle>Editar projeto</SheetTitle><SheetDescription>Atualize os dados gerais sem alterar o progresso calculado.</SheetDescription></SheetHeader><form action={updateAction} className="grid gap-5 px-6"><ProjectFields project={project} clients={clients} responsibles={responsibles} /><Feedback state={updateState} /><div><Submit>Salvar alterações</Submit></div></form><SheetFooter className="mt-8 border-t"><div><p className="font-medium">Excluir projeto</p><p className="text-xs text-muted-foreground">Também remove definitivamente todas as tarefas vinculadas.</p></div><form action={deleteAction}><Submit destructive><Trash2 />Excluir projeto</Submit></form><Feedback state={deleteState} /></SheetFooter></SheetContent>
  </Sheet>
}

export function ProjectPortfolio({ projects, clients, responsibles, isAdmin }: { projects: Project[]; clients: UserOption[]; responsibles: UserOption[]; isAdmin: boolean }) {
