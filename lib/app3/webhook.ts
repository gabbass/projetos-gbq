...
import type { App3WebhookEvent } from "./types.ts"

export const APP3_WEBHOOK_TOLERANCE_MS = 5 * 60 * 1000

export function verifyApp3Webhook(input: { rawBody: string; timestamp: string | null; signature: string | null; secret: string; now?: number }) {
  const timestamp = Number(input.timestamp)
  const now = input.now ?? Date.now()
  if (!input.signature || !Number.isFinite(timestamp) || Math.abs(now - timestamp) > APP3_WEBHOOK_TOLERANCE_MS) return false
...
export function parseApp3Webhook(rawBody: string): { kind: "known"; event: App3WebhookEvent } | { kind: "unknown"; id: string; event: string } | null {
  let value: unknown
  ...
  return { kind: "known", event: value as App3WebhookEvent }
}
